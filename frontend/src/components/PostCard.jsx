      ...
      <div className="card-body gap-3 p-4">
        ...
        {post.img ? (
          <img
            src={post.img}
            alt=""
            className="w-full max-h-[520px] object-cover rounded-box border border-base-300"
          />
        ) : null}
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={`btn btn-sm ${
                likedByMe ? "btn-secondary" : "btn-ghost"
              }`}
              onClick={() => like.mutate()}
        ...
        <div
          className={`collapse collapse-arrow bg-base-200 border border-base-300 ${
            openComments ? "collapse-open" : ""
          }`}
        >
          <div
            className="collapse-title text-sm font-medium cursor-pointer flex justify-between items-center"
            onClick={() => setOpenComments((prev) => !prev)}
          >
            <span>Comments</span>
          </div>
            ...
            <div className="space-y-3">
              ...
              {post.comments?.length ? (
                <ul className="space-y-2">
                  {post.comments.map((c) => (
                    <li
                      key={c._id ?? `${c.user?._id}-${c.text}`}
                      className="p-3 rounded-box bg-base-100 border border-base-300"
                    >
